...
- [ ] Chamar métodos como `registerUser()` e `findUserByCpf()`
...
### 2.4 Story 4: Busca de Usuário por CPF

**Como** desenvolvedor,
**Quero** buscar usuários por CPF através do SDK,
**Para que** eu possa implementar funcionalidades de consulta.

**Critérios de Aceite:**
- [ ] Chamar `conexaoAuthService.findUserByCpf(cpf)` com CPF (11 dígitos)
- [ ] SDK envia GET para `/api/v1/usuarios/cpf/{cpf}` do Auth Server
- [ ] Retornar `UserResponse` com dados do usuário
- [ ] Lançar `ResourceNotFoundException` se usuário não encontrado (404)
...
| RF-032 | Busca por CPF | SDK deve expor método `findUserByCpf(String cpf)` |
...
- [ ] CA-S1-5: Métodos `registerUser()` e `findUserByCpf()` funcionam corretamente
...
#### Story 4: Busca de Usuário por CPF

- [ ] CA-S4-1: `findUserByCpf(cpf)` retorna usuário encontrado
- [ ] CA-S4-2: CPF inválido lança validação
...
| Épico 4: Auth Service | registerUser, findUserByCpf | P1 | 1 dia |
